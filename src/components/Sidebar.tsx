...
export function Sidebar({ selectedChapter, onChapterSelect }: SidebarProps) {
  ...
  const renderChapter = (chapter: Chapter) => {
    ...
    if (searchTerm && !chapter.title.toLowerCase().includes(searchTerm.toLowerCase())) {
      return null;
    }
  ...
  const getSectionProgress = useMemo(() => {
    return (chapters: Chapter[]) => {
      ...
      return { completed, total: chapters.length, percentage: Math.round((completed / chapters.length) * 100) };
    };
  }, [progress]);
  ...
  return (
    <aside className="w-72 sm:w-80 h-full bg-gray-50/50 dark:bg-gray-900/50 border-r border-gray-200/50 dark:border-gray-700/50 flex flex-col backdrop-blur-sm">
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        <div className="space-y-3">
          {sections.map(section => {
            ...
            const filteredChapters = searchTerm 
              ? section.chapters.filter(ch => ch.title.toLowerCase().includes(searchTerm.toLowerCase()))
              : section.chapters;
            
            if (searchTerm && filteredChapters.length === 0) return null;
            
            return (
              <div key={section.id} className="card animate-slide-in hover-lift">
                <button
                ...
                {!isCollapsed && (
                  <div className="px-4 pb-4 space-y-2">
                    {(searchTerm ? filteredChapters : section.chapters).map(renderChapter)}
                  </div>
                )}
              </div>
            );
        ...
        {searchTerm && sections.every(section => 
          !section.chapters.some(ch => ch.title.toLowerCase().includes(searchTerm.toLowerCase()))
        ) && (
          <div className="text-center py-8">
            <svg className="mx-auto h-12 w-12 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.172 16.172a4 4 0 015.656 0M9 12h6m-6-4h6m2 5.291A7.962 7.962 0 0112 15c-2.34 0-4.29-1.009-5.824-2.563M15 6.494A7.966 7.966 0 0112 6c-2.34 0-4.29 1.009-5.824 2.563" />
            </svg>
