            ...
            <div className="flex-1">
              <div className="flex items-center space-x-3 mb-2">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-sm text-muted">{t('chapterView.ostepCurriculum')}</span>
                </div>
              </div>
              ...
              <div className="flex items-center space-x-4 mb-6">
                ...
                <div className={`inline-flex items-center space-x-1 px-2 py-1 rounded-md text-sm font-medium ${currentStatus === 'completed'
                    ? 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300'
                    : currentStatus === 'in-progress'
                      ? 'bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-300'
                      : 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400'
                  }`}>
                  {statusButtons.find(btn => btn.status === currentStatus)?.icon}
                  <span>{statusButtons.find(btn => btn.status === currentStatus)?.label}</span>
                </div>
              </div>
                ...
                {statusButtons.map(({ status, label, color, icon }) => {
                  ...
                  return (
                    <button
                      ...
                      className={`p-3 rounded-lg border transition-all duration-200 hover:scale-[1.02] active:scale-[0.98] ${isActive
                          ? `border-${color}-300 dark:border-${color}-600 bg-${color}-50 dark:bg-${color}-900/20 shadow-md`
                          : 'border-gray-200 dark:border-gray-700 hover:border-gray-300 dark:hover:border-gray-600 bg-white dark:bg-gray-800'
                        }`}
                      style={isActive ? {
                        borderColor: color === 'green' ? '#86efac' : color === 'yellow' ? '#fde68a' : '#d1d5db',
                        backgroundColor: color === 'green' ? '#f0fdf4' : color === 'yellow' ? '#fefce8' : '#f9fafb',
                        ...(document.documentElement.classList.contains('dark') && {
                          borderColor: color === 'green' ? '#166534' : color === 'yellow' ? '#b45309' : '#374151',
                          backgroundColor: color === 'green' ? '#14532d20' : color === 'yellow' ? '#78350f20' : '#1f293720'
                        })
                      } : {}}
                    >
                      <div className="flex items-center space-x-2">
              ...
              <div className="flex items-center space-x-4">
                <div className="w-12 h-12 bg-purple-600 rounded-xl flex items-center justify-center shadow-lg group-hover:shadow-xl transition-all duration-200">
                  <svg className="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 4V2a1 1 0 011-1h8a1 1 0 011 1v2m0 0V1a1 1 0 011-1h2a1 1 0 011 1v18a1 1 0 01-1 1H5a1 1 0 01-1-1V5a1 1 0 011-1h2z" />
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 3H8a1 1 0 00-1 1v16a1 1 0 001 1h8a1 1 0 001-1V4a1 1 0 00-1-1z" />
                  </svg>
                </div>
                <div>
        ...
        <div className="card p-6">
          ...
          {chapter.pdfs.length > 0 ? (
            <div className="space-y-4">
              {chapter.pdfs.map((pdf, index) => (
                <div key={index} className="border border-gray-200 dark:border-gray-700 rounded-xl p-4">
                  <h3 className="font-semibold text-gray-900 dark:text-white mb-3">{pdf.title}</h3>
            ...
            <div className="flex flex-col items-center justify-center py-8 text-center border border-gray-200 dark:border-gray-700 rounded-xl">
              ...
              <p className="text-gray-500 dark:text-gray-400 text-lg mb-2">{t('chapterView.noReadingMaterials')}</p>
              <p className="text-gray-400 dark:text-gray-500 text-sm">{t('chapterView.noOstepChapters')}</p>
            </div>
          )}
        </div>
